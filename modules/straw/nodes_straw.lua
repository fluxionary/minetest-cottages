...
cottages.handmill_product[ci.seed_wheat] = "farming:flour 1"
--[[ some examples:
...
minetest.register_node("cottages:handmill", {
	...
	after_place_node = function(pos, placer)
		...
		meta:set_string("infotext",
			S("Private mill, powered by punching (owned by %s)"):format(meta:get_string("owner") or ""));
		meta:set_string("formspec",
		...
		if inv:room_for_item("flour", product_stack:get_name() .. " " .. tostring(anz_result)) then
			...
			if anz_left > 0 then
				minetest.chat_send_player(name,
					S("You have ground a %s (%s are left)."):format(stack1:get_definition().description,
						(anz_left)));
			else
				minetest.chat_send_player(name,
					S("You have ground the last %s."):format(stack1:get_definition().description));
			end
